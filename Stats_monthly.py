...
def calcmon_ma_ml_mh(qm,qy,par):
    ...
    qm_nz = qm[qm['q']>=zeroflows]['q'].to_numpy()
    pcflows = calc.flowpercentiles(qm_nz,parray)                               # cacluate percentiles of nonzero flows
    qmnz_log10 = np.where(qm_nz > 0, qm_nz, zeroflows)                         # Log10 of flows, exchange the zeros for a positive value
    qmnz_log10 = np.log10(qmnz_log10)
    pclogflows = calc.flowpercentiles(qmnz_log10,parray)                       # cacluate percentiles of log10 nonzero flows
    
    parray2 = np.array([1,10,25,50])                                           # set up percentile array for mh15-16-17
    pcflows2 = calc.flowpercentiles(qm_nz,parray2)                             # cacluate percentiles of nonzero flows
    ...
    ma[1] = qm['q'].mean()                                                     # mean of all flows
    ma[2] = qm['q'].median()                                                   # median of all flows
    ...
    ma[36] = scalardiv((qm['q'].max() - qm['q'].min()) , qm['q'].median())
    ma[37] = scalardiv((pcflows[4] - pcflows[14]) , pcflows[9])                # (25th - 75th) / 50th
    ma[38] = scalardiv((pcflows[1] - pcflows[17]) , pcflows[9])                # (10th - 90th) / 50th
    ma[39] = qm['q'].std(ddof=1) * 100 / qm['q'].mean()
    ma[40] = scalardiv((qm['q'].mean() - qm['q'].median()) , qm['q'].median())
    ...
    mh[18] = (np.log10(qy['max'].clip(lower=zeroflows))).std(ddof=1) * 100 / (np.log10(qy['max'].clip(lower=zeroflows))).mean()
    ...
    hfthresh2 = pcflows[4]
    a1,qvol1,qmax1,b,c,d = calc.spellsabove(qmq,qmm,qmm,qmy,hfthresh1)         # stats for events above median x 1
    a3,qvol3,qmax3,b,c,d = calc.spellsabove(qmq,qmm,qmm,qmy,hfthresh1*3)       # stats for events above median x 3
    a7,qvol7,qmax7,b,c,d = calc.spellsabove(qmq,qmm,qmm,qmy,hfthresh1*7)       # stats for events above median x 7
    a75,qvol75,qmax75,b,c,d = calc.spellsabove(qmq,qmm,qmm,qmy,hfthresh2)      # stats for events above 75th (=25th exceedance)
    ...
    for mon in range(0,12):
        ...
        qmon = qm[qm.index.month==(mon+1)]['q'].to_numpy()
        ma[iavg] = meanORmed(qmon,par['opt_median'])                           # ma12-23 = average total flow in each month
        ma[icv] = scalardiv(np.std(qmon,ddof=1) , np.mean(qmon)) * 100         # ma24-35 = cv of flow in each month
...
def calcmon_fl_fh(qm,qy,par):
    ...
    thresh = np.percentile(qm_nz,25)                                           # 25th pc (=75th exceedance)   <====
    starts,spellon,annstarts,anndays = calc.spellsbelow(qmq,qmm,qmm,qmy,thresh) # spell stats
    ...
    thresh = np.mean(qmq) * 0.05                                               # 5pc of mean daily flow
    starts,spellon,annstarts,anndays = calc.spellsbelow(qmq,qmm,qmm,qmy,thresh) # spell stats
    if np.sum(annstarts)>0:
    ...
    thresh = np.percentile(qm_nz,75)                                         # 75th pc (=25th exceedance)   <====
    starts,qvol,qmax,spellon,annstarts,anndays = calc.spellsabove(qmq,qmm,qmm,qmy,thresh)   # spell stats
    if np.sum(annstarts)>0:
        ...
        fh[1] = fh[2] = fh[8] = 0.0
                        ...
                        # fh1 and fh8 appear to give IDENTICAL RESULTS in the EflowStats R code
                        # But... fh1 and fh9 (not fh8) have IDENTICAL DESCRIPTIONS in the HIT user manual
                        # I have decided to match the R code and IGNORE THE MANUAL for fh1 / fh8 / fh9
                        # ie. I have made fh1 = fh8
                        #=============================
    
    thresh = np.median(qm_nz) * 3.0                                            # median daily flow x 3         <====
    starts,qvol,qmax,spellon,annstarts,anndays = calc.spellsabove(qmq,qmm,qmm,qmy,thresh)   # spell stats
    if np.sum(annstarts)>0:
    ...
    thresh = np.median(qm_nz) * 7.0                                            # median daily flow x 7         <====
    starts,qvol,qmax,spellon,annstarts,anndays = calc.spellsabove(qmq,qmm,qmm,qmy,thresh)   # spell stats
    if np.sum(annstarts)>0:
    ...
    thresh = np.median(qm_nz)                                                  # median daily flow             <====
    starts,qvol,qmax,spellon,annstarts,anndays = calc.spellsabove(qmq,qmm,qmm,qmy,thresh)   # spell stats
    if np.sum(annstarts)>0:
    ...
    thresh = np.percentile(qm_nz,25)                                         # 75th pc (=25th exceedance)    <====
    starts,qvol,qmax,spellon,annstarts,anndays = calc.spellsabove(qmq,qmm,qmm,qmy,thresh)   # spell stats
    if np.sum(annstarts)>0:
...
def calcmon_dl_dh(qm,qy,par):
    ...
    qm_nzmed = np.median(qm_nz)
    
    # rolling stats -----------------------------------------------------------
    min_1m = qm['q'].resample('AS').min()
    max_1m = qm['q'].resample('AS').max()
    min_3m = qm['q'].rolling(3,center=True).mean().resample('AS').min()
    max_3m = qm['q'].rolling(3,center=True).mean().resample('AS').max()
    
    # low flow stats ----------------------------------------------------------
    dl[4] = meanORmed(min_1m[1:-1],par['opt_median'])                          # average minima
    dl[5] = meanORmed(min_3m[1:-1],par['opt_median'])                          # [1:-1] strips out the first and last values
    
    dl[9] = scalardiv(np.std(min_1m[1:-1],ddof=1), np.mean(min_1m[1:-1])) * 100  # variability of minima
    dl[10] = scalardiv(np.std(min_3m[1:-1],ddof=1), np.mean(min_3m[1:-1])) * 100
    
    dl[13] = scalardiv(np.mean(min_1m[1:-1]) , qm_nzmed)
    
    thresh = np.percentile(qm_nz,25)                                           # 25th pc (=75th exceedance)   <====
    dl[14] = scalardiv(thresh , qm_nzmed)
    
    thresh = np.percentile(qm_nz,10)                                           # 10th pc (=90th exceedance)   <====
    dl[15] = scalardiv(thresh , qm_nzmed)
    ...
    thresh = par['zeroflowmonth']                                                # spells of zero flow          <====
    starts,spellon,annstarts,anndays = calc.spellsbelow(qmq,qmm,qmm,qmy,thresh)    # spell stats
    if np.sum(annstarts)>0:
    ...
    dl[20] = len(qm[qm['q']<=thresh]) / len(qm)                                # zero months / total months
    
    # high flow stats -------------------------------------------------------------------
    dh[4] = meanORmed(max_1m[1:-1],par['opt_median'])                          # average maxima
    dh[5] = meanORmed(max_3m[1:-1],par['opt_median'])
    
    dh[9] = np.std(max_1m[1:-1],ddof=1) * 100 / np.mean(max_1m[1:-1])          # variability of maxima
    dh[10] = np.std(max_3m[1:-1],ddof=1) * 100 / np.mean(max_3m[1:-1])
    
    dh[13] = scalardiv(np.mean(max_1m[1:-1]) , qm_nzmed)
    
    dh[14] = qm['q'].quantile(0.95) / qm['q'].mean()
    ...
    starts,qvol,qmax,spellon,annstarts,anndays = calc.spellsabove(qmq,qmm,qmm,qmy,thresh)   # spell stats
    dh[17] = 0.0
    if np.sum(annstarts)>0: dh[17] = meanORmed(starts[:,0],par['opt_median'])  # mean event duration (col 0)

    thresh = np.median(qm_nz) * 3                                              # median daily flow x 3        <====
    starts,qvol,qmax,spellon,annstarts,anndays = calc.spellsabove(qmq,qmm,qmm,qmy,thresh)   # spell stats
    dh[18] = 0.0
    if np.sum(annstarts)>0: dh[18] = meanORmed(starts[:,0],par['opt_median']) # mean event duration (col 0)

    thresh = np.median(qm_nz) * 7                                              # median daily flow x 7        <====
    starts,qvol,qmax,spellon,annstarts,anndays = calc.spellsabove(qmq,qmm,qmm,qmy,thresh)   # spell stats
    dh[19] = 0.0
    if np.sum(annstarts)>0: dh[19] = meanORmed(starts[:,0],par['opt_median'])  # mean event duration (col 0)

    thresh = np.percentile(qm_nz,75)                                          # 75th pc (=25th exceedance)   <====
    starts,qvol,qmax,spellon,annstarts,anndays = calc.spellsabove(qmq,qmm,qmm,qmy,thresh)   # spell stats
    dh[20] = 0.0
    if np.sum(annstarts)>0: dh[20] = meanORmed(starts[:,0],par['opt_median'])  # mean event duration (col 0)

    thresh = np.percentile(qm_nz,25)                                           # 25th pc (=75th exceedance)   <====
    starts,qvol,qmax,spellon,annstarts,anndays = calc.spellsabove(qmq,qmm,qmm,qmy,thresh)   # spell stats
    dh[21] = 0.0
    if np.sum(annstarts)>0: dh[21] = meanORmed(starts[:,0],par['opt_median'])  # mean event duration (col 0)
    ...
    starts,spellon,annstarts,anndays = calc.spellsbelow(qmq,qmm,qmm,qmy,thresh) # spell stats BELOW the flood
    dh[24] = 0.0
    if np.sum(annstarts)>0: dh[24] = meanORmed(anndays,par['opt_median'])                              # flood free days (mean days per yr)
...
def calcmon_ta_tl_th(qm,qy,par):
    ...
    logmeanflow = np.log10(np.mean(qmq))
    ...
    spellon_sr = pd.Series(spellon,index=qm.index)                             # get series of spellon (binary spell on/off = 1/0)
    hist_1m = spellon_sr.groupby(spellon_sr.index.month).sum()                 # histogram of num spell months for each 1 month window
    spellon_2m = spellon_sr.resample('2MS').sum()                              # resample to 2m intervals
    hist_2m = spellon_2m.groupby(spellon_2m.index.month).sum()                 # histogram of num spell months for each 2 month window
    
    ta[3] = hist_2m.max() / hist_2m.sum()                                      # major spell window / all windows
    
    # high/low flow day ------------------------------------------------------
    annvol,annmax,annmin,annmaxday,annminday,anncvs = calc.annual_stats(qmq,qmy,qmm) # annual stats

    tl[1] = scistat.circmean(annminday,high=12,low=1)
    tl[2] = scistat.circstd(annminday,high=12,low=1)
    
    th[1] = scistat.circmean(annmaxday,high=12,low=1)
    th[2] = scistat.circstd(annmaxday,high=12,low=1)
...
def calcHITMonthly(flow_df,opt_median,catskm2,units):
    ...
    for icat,cat in enumerate(catchments):
        ...
        #single_flow = single_flow.resample('MS').sum()                         # aggregate to total monthly flow
        ...
        if (single_flow.max() - single_flow.min() > 1):                        # If its a valid series (not all 0's or 5's)
            ...
            qm = pd.DataFrame(single_flow)                                     # basic daily flow dataframe
            qm.columns = ['q']
            qm['y'] = qm.index.year
            qm['m'] = qm.index.month
            ...
            ari = 1.666667
            par['floodthresh'] = calc.aepthresh(qm['q'].to_numpy(),
                                                qm['y'].to_numpy(),
                                                ari,'high')                    # 1.67yr flood thresh (60%ile) using ann series method
            ari = 5
            par['drythresh'] = calc.aepthresh(qm['q'].to_numpy(),
                                              qm['y'].to_numpy(),
            ...
            #print('    ma, ml, mh')
            t3 = time.time()
            ma, ml, mh = calcmon_ma_ml_mh(qm,qy,par)

            # Low flow and high flow stats ------------------------------------
            #print('    fl, fh')
            t4 = time.time()
            fl, fh = calcmon_fl_fh(qm,qy,par)
            
            # Duration stats --------------------------------------------------
            #print('    dl, dh')
            t5 = time.time()
            dl, dh = calcmon_dl_dh(qm,qy,par)
            
            # Timing stats ----------------------------------------------------
            #print('    ta, tl, th')
            t6 = time.time()
            ta, tl, th = calcmon_ta_tl_th(qm,qy,par)
            
            # Rise and fall stats ---------------------------------------------
            #print('    ra')
            t7 = time.time()
            ra = calcmon_ra(qm,qy,par)
